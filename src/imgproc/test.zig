...
test "imgproc findContours" {
    ...
    try testing.expectEqual(@as(i32, 1436), @intFromFloat(length));

    length = imgproc.arcLength(r0, false);
    try testing.expectEqual(@as(i32, 1037), @intFromFloat(length));
}
...
test "imgproc rectangleWithParams" {
    ...
    for (tests) |t| {
        ...
        try testing.expectEqual(@as(u8, 255), img.get(u8, @intCast(t.point.x), @intCast(t.point.y)));
    }
}
...
test "imgproc ellipse" {
    ...
    for (tests) |tc| {
        ...
        try testing.expectEqual(@as(u8, 255), img.get(u8, @intCast(tc.point.x), @intCast(tc.point.y)));
    }
}
...
test "imgproc ellipseWithParams" {
    ...
    for (tests) |tc| {
        ...
        const r = img.get(u8, @intCast(tc.point.x), @intCast(tc.point.y));
        if (tc.want) |want| {
...
test "imgproc fillPolyWithParams" {
    ...
    for (tests) |tc| {
        ...
        try testing.expectEqual(tc.result, img.get(u8, @intCast(tc.point.x), @intCast(tc.point.y)));
    }
}
